...
if (strncmp(buffer, "POST /upload", 12) == 0) {
    ...
    if (content_length_index > 0) {
        int content_length = atoi(&buffer[content_length_index]);
        char *body_start = strstr(buffer, "\r\n\r\n") + strlen("\r\n\r\n");
        int remaining_data = valread - (body_start - buffer);

        FILE *fileptr = fopen("received_file.txt", "wb");
        fwrite(body_start, sizeof(char), remaining_data, fileptr);

        // Tiếp tục đọc dữ liệu cho đến khi nhận được toàn bộ file
        while (remaining_data < content_length) {
            valread = read(new_socket, buffer, BUFFER_SIZE);
            fwrite(buffer, sizeof(char), valread, fileptr);
            remaining_data += valread;
        }
